...
class OpenIDConnectClient
{
    ...
    public function authenticate() {
        ...
        if (isset($_REQUEST["code"])) {
            ...
            $code = $_REQUEST["code"];
            $this->tokenJson = $this->requestTokens($code);

            // Throw an error if the server returns one
            if (isset($this->tokenJson->error)) {
                if (isset($this->tokenJson->error_description)) {
                    throw new OpenIDConnectClientException($this->tokenJson->error_description);
                }
                throw new OpenIDConnectClientException('Got response: ' . $this->tokenJson->error);
            }
	    ...
	    $this->unsetState();
            ...
            if (!property_exists($this->tokenJson, 'id_token')) {
                throw new OpenIDConnectClientException("User did not authorize openid scope.");
            }

            $claims = $this->decodeJWT($this->tokenJson->id_token, 1);
		...
		if (!$this->getProviderConfigValue('jwks_uri')) {
                ...
                if (!$this->verifyJWTsignature($this->tokenJson->id_token)) {
		...
		// Save the full response
                $this->tokenResponse = $this->tokenJson;

                // Save the id token
                $this->idToken = $this->tokenJson->id_token;

                // Save the access token
                $this->accessToken = $this->tokenJson->access_token;

                // Save the refresh token, if we got one
                $this->refreshToken = $this->tokenJson->access_token;
 ...
 /**
    ...
    public function getTokenEndpoint() {
        ...
        if (!isset($this->providerConfig['token_endpoint'])) {
            throw new OpenIDConnectClientException("The token endpoint has not been discovered");
        } else {
            return $this->providerConfig['token_endpoint'];
        }
    }
